...
>
    ...
    >
        ...
        <span class="support-chat-launcher-copy">
            ...
            <strong>{{ __('store.support.launcher') }}</strong>
        </span>
        <span class="support-chat-badge" data-chat-badge hidden aria-label="{{ __('store.support.unread', ['count' => 0]) }}">0</span>
    </button>
    ...
    >
        <header class="support-chat-header">
            ...
            <div class="support-chat-heading">
                ...
                <h2 id="lunar-support-title">{{ __('store.support.title') }}</h2>
                <p class="support-chat-status" data-chat-status data-status="bot">
                    <span aria-hidden="true"></span>
                    <b>{{ __('store.support.assistant') }}</b>
                </p>
            </div>
            <button class="support-chat-close" type="button" data-chat-close aria-label="{{ __('store.support.close') }}">
        ...
        <div class="support-chat-body">
            <p id="lunar-support-privacy" class="support-chat-privacy">
                {{ __('store.support.privacy') }}
            </p>

            <div class="support-chat-error" data-chat-error role="alert" hidden>
                <span data-chat-error-text>{{ __('store.support.connection_error') }}</span>
                <button type="button" data-chat-retry>{{ __('store.support.retry') }}</button>
            </div>
            ...
            >
                <div class="support-chat-loading" data-chat-loading>
                    <span aria-hidden="true"></span>
                    <p>{{ __('store.support.connecting') }}</p>
                </div>
                <div class="support-chat-empty" data-chat-empty hidden>
                    <span class="support-chat-empty-mark" aria-hidden="true">✦</span>
                    <strong>{{ __('store.support.welcome') }}</strong>
                    <p>{{ __('store.support.welcome_copy') }}</p>
                </div>
            </div>

            <div class="support-chat-suggestions" data-chat-suggestions hidden aria-label="{{ __('store.support.suggestions') }}"></div>
        </div>

        <form class="support-chat-composer" data-chat-form novalidate>
            <label class="sr-only" for="lunar-support-message">{{ __('store.support.message_label') }}</label>
            <div class="support-chat-compose-row">
                <textarea
                    ...
                    rows="1"
                    maxlength="2000"
                    placeholder="{{ __('store.support.message_placeholder') }}"
                    autocomplete="off"
                ></textarea>
                <button class="support-chat-send" type="submit" data-chat-send aria-label="{{ __('store.support.send') }}" disabled>
                    <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
            ...
            <div class="support-chat-actions">
                <button type="button" data-chat-handoff>
                    <span aria-hidden="true"></span>
                    {{ __('store.support.specialist') }}
                </button>
                <small data-chat-composer-note>{{ __('store.support.note') }}</small>
            </div>
        </form>
