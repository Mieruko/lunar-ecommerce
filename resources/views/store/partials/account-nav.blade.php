<aside class="account-sidebar">
    ...
    <nav class="account-nav account-nav-expanded" aria-label="{{ __('store.common.account') }}">
        ...
        <a @class(['active' => request()->routeIs('account.notifications*')]) href="{{ route('account.notifications') }}"><span>03</span> {{ __('store.notifications.title') }} @if(auth()->user()->unreadNotifications()->count())<b class="account-nav-badge">{{ auth()->user()->unreadNotifications()->count() }}</b>@endif</a>
